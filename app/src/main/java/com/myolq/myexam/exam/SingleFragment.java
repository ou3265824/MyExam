...
//        vpPager.setAdapter(examAdapter);
        singleDao = new SingleDao(AppUtils.appContext);
        setTime();
        setHandler();
        if (!activity.isView()){
...
//                int time=60*60;
    ...
    public View getViewSingle(SingleBean single) {
        View view = LayoutInflater.from(AppUtils.appContext).inflate(R.layout.view_single, null);
        setinit(view, single);
        return view;
    }
...
//                rbA.setButtonDrawable(android.support.v7.appcompat.R.drawable.abc_btn_radio_to_on_mtrl_000);
//                rbB.setButtonDrawable(new BitmapDrawable(bitmap));
//                rbC.setButtonDrawable(new BitmapDrawable(bitmap));
//                rbD.setButtonDrawable(new BitmapDrawable(bitmap));
            }
...
//            }
//        });
        DialogUtils.createAlertDialog(AppUtils.appContext, "分数","你考了" + grade + "分", new DialogInterface.OnClickListener() {
            @Override
